...
//trzeba poczekac na przeciwnika    
    ...
    public void followOrders(Message m)
    {
    	switch(m.getOrder())
    	{
    		...
    		case DENY://ruch odrzucony
    			...
    			frame.setActive();// powtórz ruch
    			waitForOrder();//czekaj na akcjê u¿ytkownika
    			break;
    		case NEXT_PHASE:
    			Report results = new Report(m.getX()+5, m.getY());
    			System.out.println("Nastêpna faza.");
    		...
    		case SET_DEAD://drugi gracz ustawi³ pole na martwe
    			...
    			frame.setTable(m.getTable());//zmieñ kolor pola pole
    			frame.refresh();
    			boolean acceptance = frame.askForAcceptance();		//zapytaj o akceptacjê
    ...
    public void waitForOrder()
    {
    	...
    	switch(frame.getOrder())
    	{
    		...
    		case PASS:
    			...
    			break;
    		case SET_DEAD://ustaw martwy kamieñ
    			...
    			break;
    		case END_TURN://koñczê swoj¹ kolejkê w ustawianiu martwych kamieni
	...
	public void negotiate(Phase p)
	{
		...
		while(phase == p)
		{
			...
			try
			{
				m = (Message)clInput.readObject();//czekaj na wiadomoœc (akceptacja, odrzucenie)
