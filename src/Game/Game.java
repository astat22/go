 ...
 */
	...
	private void mergeChains(List<Chain> listOfChains, boolean mergeAnyway) throws CannotMergeChainsException
	{
		...
		if(newChain.getBreaths()>0 || mergeAnyway)//nowy ³añcuch ma wystarczaj¹c¹ liczbê oddechów
		{
			for(Chain c: listOfChains)
			{
	...
	/** Przeci¹¿ona metoda zwraca listê ³añcuchów do usuniêcia, w zale¿noœci od tego, czy maj¹ 1, czy 0 oddechów.
	 * 
	 */
	private List<Chain> checkForDeadChains(boolean almostDead, Square n,Colour col)
	{
		List<Chain> chainsToRemove = new ArrayList<>();//trzeba utworzyc kopiê, inaczej ConcurrentModificationException
		int deadlyBreaths = almostDead?1:0;
		for(Chain c: chains)
		{
			c.computeNeighbours();
			...
			RepeatedSituationException
	{
		...
		else
		{
			...
			if(initialBreathCheking(place, colour)) //czy po dodaniu kamienia bêdzie oddech? 
			{
				...
				checkForDeadChains(false,colour);// licz¹ siê tylko wrogie ³añcuchy!//TODO czy teraz dzia³a?
			}
			else
			{
				...
				if(chainsToRemove.size()>0) // zasada nie dzia³a, jeœli po dodaniu kamienia powtórzy³aby siê sytuacja previousBoard
				{
					...
					if(simulatedGame.getSquareByCoordinates(place.getxCoord(), place.getyCoord()).getColour()==Colour.NONE)
						System.out.println("Symulacja nie powiod³a siê!");
				...
				for(Square s:ptrSquares)
				{
					if(s.getColour()==Colour.NONE && !s.isMarked())//bierzemy pod uwagê puste miejsca, które nie s¹ oznaczone
					{
						try
						{
							...
							//System.out.println("Dodajê nowe terytorium");
						}
